...
.. [#zip] The operation that forms the sequence :math:`\isequence{(\texttt{"Waveforms"}, \texttt{"Pedestals"})}{\text{APA}}` from the separate sequences :math:`\isequence{\texttt{"Waveforms"}}{\text{APA}}` and :math:`\isequence{\texttt{"Pedestals"}}{\text{APA}}` is called *zip*.
